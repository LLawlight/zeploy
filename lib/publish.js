...
module.exports = async (env) => {
  ...
  const { ssh, distPath, targetPath, keepReleases = 10, checkBranch } = config

  const currentBranchName = execSync('git rev-parse --abbrev-ref HEAD').toString().trim();

  if (checkBranch && currentBranchName !== env) {
    log.fail('当前分支不允许执行此发布模式！')
    return
  }
...
> 服务器发布路径：${config.targetPath}
  ...
  read({
    ...
    if (text == 'yes') {
      ...
      // 创建文件夹
      await connExecMkdir(targetDirPaths)
      ...
      //   console.log(JSON.stringify(distPath))
...
function connExecMkdir(targetDirPaths) {
  return new Promise((res) => {
    conn.exec(`mkdir -p ${targetDirPaths}`, function(err, stream) {
      if (err) {
        rej(err)
...
function getAllPath(distPath) {
  ...
  console.log(JSON.stringify(dirList), JSON.stringify(fileList))
